...
// Based on https://sourceforge.net/p/sevenzip/discussion/45797/thread/ec3effac/#0d3a/610e
bool isLzmaHeader(const uint8_t* data, size_t size) {
    ...
    const uint64_t MAX_UNCOMPRESSED_SIZE = (1ULL << 32); // why the hell would a ramdisk be bigger than 4gb.
    if (uncompressedSize != 0xFFFFFFFFFFFFFFFFULL && uncompressedSize > MAX_UNCOMPRESSED_SIZE) {
        return false;
    }
