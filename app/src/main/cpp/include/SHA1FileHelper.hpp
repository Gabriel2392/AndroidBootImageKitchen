...
bool ValidateSHA1(fs::path& filename) noexcept {
    ...
    if (filesize < static_cast<std::streamoff>(SHA1_DIGEST_SIZE))
        return false;

    const std::streamoff content_size = filesize - static_cast<std::streamoff>(SHA1_DIGEST_SIZE);
    ...
    while (remaining > 0 && file) {
        const auto to_read = static_cast<std::streamsize>(std::min<std::streamoff>(buffer_size, remaining));
