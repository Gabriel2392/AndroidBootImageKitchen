...
UnpackBootImage(int fd, const std::filesystem::path &output_dir, bool dec_ramdisk) {
  ...
  if (info.header_version < 3) {
    ...
    LOG("Secondary bootloader size: %.2fMB", static_cast<float>(info.second_size) / 1024 / 1024);
    info.second_load_address = kernel_ramdisk_second_info[5];
    info.tags_load_address = kernel_ramdisk_second_info[6];

    if (!utils::ReadU32(fd, os_version_patch_level))
      return std::nullopt;
  } else {
    info.kernel_size = kernel_ramdisk_second_info[0];
  ...
  LOG("Kernel size: %.2fMB", static_cast<float>(info.kernel_size) / 1024 / 1024);
  LOG("Ramdisk size: %.2fMB", static_cast<float>(info.ramdisk_size) / 1024 / 1024);
  ...
  if (info.header_version == 1 || info.header_version == 2) {
    if (!utils::ReadU32(fd, info.recovery_dtbo_size))
      return std::nullopt;
    LOG("Recovery DTBO size: %.2fMB", static_cast<float>(info.recovery_dtbo_size) / 1024 / 1024);
    if (!utils::ReadU64(fd, info.recovery_dtbo_offset))
      return std::nullopt;
    if (!utils::ReadU32(fd, info.boot_header_size))
      return std::nullopt;
  }

  if (info.header_version == 2) {
    if (!utils::ReadU32(fd, info.dtb_size))
      return std::nullopt;
    LOG("DTB size: %.2fMB", static_cast<float>(info.dtb_size) / 1024 / 1024);
    if (!utils::ReadU64(fd, info.dtb_load_address))
      return std::nullopt;
  }

  if (info.header_version >= 4) {
    if (!utils::ReadU32(fd, info.boot_signature_size))
      return std::nullopt;
  }
  ...
  uint32_t num_kernel_pages =
      GetNumberOfPages(info.kernel_size, page_size);
  ...
  uint32_t num_ramdisk_pages =
      GetNumberOfPages(info.ramdisk_size, page_size);
  if (info.ramdisk_size > 0) { // Patch2: Only unpack ramdisk if it exists
    ...
    image_entries.emplace_back(ramdisk_offset,
